...
def get_train_samples(args, sample_data, custom_steps=None,start_step=0,end_step=100,step_size=2):
    ...
    if num_st == 1:
        ...
        timesteps = list(range(start_step, end_step, step_size))
...
def resume_cali_model(qnn, ckpt_path, cali_data, quant_act=False, act_quant_mode='qdiff', cond=False,resume_from_recon=False):
    ...
    if quant_act:       
        ...
        ckpt = torch.load(ckpt_path, map_location='cpu')
        if resume_from_recon:
            ckpt = {'model.' + key: value for key, value in ckpt['model_state'].items()}
